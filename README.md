...
- 订单列表、订单详情、撤销订单（可选择撤销到小程序收银台或桌面端收银台）
- 采集中心（扫码录入、AI 拍照识别）
- 销售报表、数据分析
...
> **商品图片 → AI 样本自动同步**
> 
> 上传/更新商品图片时，系统会自动将该图片复制到 AI 样本目录（`data/samples/sku_XXX/product_img.xxx`），作为 AI 识别的一张样本。
> 
> | 操作 | 商品图片 | AI 样本 |
> |------|---------|---------|
> | 上传图片 | 保存 | 自动同步一份 |
> | 更新图片（传错了重新传） | 替换 | 自动替换，不产生冗余 |
> | 删除图片 | 删除 | 同步删除 |
> | 手动上传的样本（img_01, img_02...） | 不影响 | 不影响 |
> 
> **已有商品图片一键同步**：如果之前已经上传过商品图片，可以调用接口一次性全部同步：
> ```bash
> curl -X POST http://localhost:8000/products/sync_images_to_samples -H "X-API-Key: 你的密码"
> ```
> 已同步过的商品会自动跳过，不会重复。
...
- **手动添加商品** - 不在系统中的商品（如称重商品）可直接输入名称和金额加入购物车
- 调整数量，点击 **结账** 完成交易
...
### 采集功能
- 采集中心：扫码录入、AI 拍照识别
- AI 识别上传失败时自动重试（最多 3 次），网络波动时无需手动操作
